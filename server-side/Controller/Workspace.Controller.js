...
const Model = require('../Models/Workspace.Model')
...
module.exports = {
    ...
    list: async (req, res, next) => {
        try {
        const { firstName, is_active, page, limit, sort } = req.query
        const _page = page ? parseInt(page) : 1
        const _limit = limit ? parseInt(limit) : 20
        const _skip = (_page - 1) * _limit
        const _sort = sort ? sort : '+firstName'
        const query = {};
        if (firstName) {
    ...
    restore: async (req, res, next) => {
        ...
        const dataToBeDeleted = await Model.findOne({ _id: mongoose.Types.ObjectId(id) }, { firstName: 1 }).lean()
        if (!dataToBeDeleted) {
            throw createError.NotFound(`${ModelName} Not Found`)
        }
        const dataExists = await Model.findOne({ firstName: dataToBeDeleted.firstName, is_active: true }).lean()
        if (dataExists) {
